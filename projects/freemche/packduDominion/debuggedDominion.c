...
int buyCard(int supplyPos, struct gameState *state) {
  ...
  if (state->numBuys < 1){
    ...
    //  This is the addition to the code which I am doing to implement dominion's
    //  embargo functionality.  It is written for the debugging portion of
    //  Assignment 5.
    if (state->embargoTokens[supplyPos] != 0)
    {
	    if (DEBUG)
		    printf("You bought a card with 1 or more embargo tokens on it.\n");
	    int numEmbargo = state->embargoTokens[supplyPos];
	    int i;
	    for (i=0; i<numEmbargo; i++)
	    {
		    //  This puts the correct number of curse cards in player's discard
		int result;
		//  Normally, I would check to see if gainCard returns an error
		//  code, and return an error code from buyCard() as well.
		//  But we actually don't care if there aren't enough curse cards
		//  to gain another one.  If they run out, the game just continues
		//  without players gaining another curse card.
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	    cardEffectStatus = adventurerEffect(state);
    ...
    case remodel:
	    cardEffectStatus = remodelEffect(choice1, choice2, state, handPos);
	    return cardEffectStatus;
...
//  The adventurer card lets you go through your deck revealing and discarding
//  cards until you have come upon two treasure cards.  These you put into
//  your hand.
//  This function returns 0 for success, -1 for failure.  It changes the
//  values stored in state.
//
//  Notes:  I see some problems with this function but for now will just leave
